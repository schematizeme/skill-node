...
**Versão:** skill `schematize-node` v0.12.0. Changelog em `CHANGELOG.md`.
...
- **Todo piso de qualidade (TS estrito, teto de 750 linhas/arquivo com flag em >300 úteis, doc-comment, MAPA/índice, formatação, veto a `any`) vale para: (1) arquivo NOVO, e (2) o TRECHO que você efetivamente alterou.** Você **não** é obrigado a converter/quebrar/documentar/reformatar o arquivo ou módulo inteiro só porque encostou nele.
...
- **Piso de segurança e de EFEITO EXTERNO não é escopo-diff.** A regra escoteiro cobre piso de **qualidade** (tipagem, tamanho, doc-comment, formatação). Segredo no cliente, SQL concatenado, auth no cliente e **envio real fora de produção** valem **repo-wide e já** — o e-mail do monolith legado queima **o mesmo IP e o mesmo domínio** que o do serviço Go novo. Legado em saída **não ganha desconto de piso**.
...
| `/node-iam` | audita/planeja a **migração do auth legado Node/TS** (Passport/JWT-no-localStorage/bcrypt/email-como-ID/1 fator/monolith) → **IAM da casa (app separada, prioridade 0, strangler-fig)**: mapeia `users`→ID interno, re-hash preguiçoso, força 2º fator, revoga sessões legadas, re-deriva authz |
| `/node-ops` | audita/scaffolda o `<projeto>_ops` (interface única): fluxo de ambientes, instalação paralela (`nproc`), independência |
| `/node-review` | gate escopo-diff da DoD/anti-padrões no diff: `any`/`@ts-ignore` novo, `.catch(()=>{})`, arquivo tocado >750 (bloqueia) / >300 úteis (flag), **rota/handler/serviço Node NOVO sem ADR**, segredo de `process.env` |
| `/node-cc` · `/node-handoff` · `/node-qa` · `/node-index` | context compact / handoff / Q.A. no contexto Node/TS (aplica a schematize-qa: /qa-plan → /qa-run) / (re)gera índice |
...
| **IAM — migração de auth legado (prioridade 0): legado típico, alvo (app separada, ≥2 fatores, ReBAC, sessão longa, logout irreversível), ordem de corte strangler-fig** | `references/iam.md` |
