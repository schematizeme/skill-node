...
> `references/arquitetura.md`. Leia lá primeiro; aqui fica **só o que muda em Node/TypeScript**.
>
...
> **Canônico em `references/padroes-codigo.md`** (teto duro de 750 linhas/arquivo — ≤ ~500 de código útil + ~250 de comentário — com **flag** em > 300 linhas de código útil, uma função/unidade por arquivo, doc-comment obrigatório com motivo/comportamento/entradas/saídas/efeitos, e `MAPA.md`). Esta seção é o recorte de arquitetura desses pisos — não duplica a regra, contextualiza.

**MUST — arquivos pequenos, micro-funções**

- **Teto duro: ≤ 750 linhas/arquivo (~500 de código útil + ~250 de comentário); FLAG em > 300 de código útil.** Acima do teto o arquivo **deve ser quebrado** — extraia responsabilidades em módulos menores e a lógica em **micro-funções** com nome que explica a intenção. Passar de 300 linhas úteis não bloqueia, mas **sempre sinaliza** (indício de função extensa / falta de abstração — dívida a rever); observabilidade tem folga ~400. Não existe "arquivo de 800 linhas porque é coeso": coesão real cabe em arquivos pequenos colaborando.
...
- Arquivo de produção > 750 linhas (ou > ~500 de código útil) sem quebra (exceto as exceções acima) → bloqueia; > 300 linhas de código útil (~400 em observabilidade) → **flag** (não bloqueia, sempre sinalizado como dívida).
