...
> **Dividida:** §22.4–§23 (padrão de script, seeds, CI, pentest §22.7–22.8, Q.A. plan-first — disciplina na skill **schematize-qa**, Makefile) estão em `references/testes-execucao.md`. Numeração contínua entre os dois.
...
- **Teste NUNCA dispara efeito externo de verdade** (e-mail, SMS, push, webhook, cobrança) — **piso repo-wide, não escopo-diff**: transporte **sink** (`jsonTransport`/Mailpit), destinatário sintético `<papel>+<run-id>-<n>@test.<domain>`, e o **teste vermelho do guard** — `await assert.rejects(() => mailer.sendMail({ to: "alguem@gmail.com", ... }), (e) => e instanceof ExternalRecipientBlockedError)` com o transporte falso **vazio** — mais o teste do **cap por execução**. Código pronto em `references/iam.md` §3.1; normativa em `schematize-engineering` → `references/efeitos-externos.md`. **VETADO** `@gmail.com`/domínio de terceiro/e-mail de pessoa real em teste, fixture, seed ou script (grep no CI trava).
