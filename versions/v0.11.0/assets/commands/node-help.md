...
| `/node-iam` | Audita/planeja a **migração do auth legado Node/TS para o IAM da casa** (app separada, **prioridade 0**, strangler-fig): mapeia `users`→ID interno (email≠ID), re-hash preguiçoso (bcrypt→argon2id), JWT fora do `localStorage`, força 2º fator, revoga sessões legadas, re-deriva authz no ReBAC, logout irreversível. |
| `/node-ops` | Audita/scaffolda o `<projeto>_ops` (interface única): fluxo de ambientes (dev→local→github→hml→prd), instalação paralela (`nproc`), independência dos serviços. |
| `/node-review` | Gate **escopo-diff** da Definition of Done/anti-padrões no diff: `any`/`@ts-ignore` **novo**, `.catch(()=>{})`, arquivo tocado >750 (bloqueia) / >300 úteis (flag), **rota/handler/serviço Node NOVO sem ADR**, segredo ecoado de `process.env`. |
...
| `/node-qa` | Q.A. no contexto Node/TS (aplica a schematize-qa: /qa-plan → /qa-run). |
