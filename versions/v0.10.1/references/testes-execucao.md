...
> Parte da skill **schematize-node**. Continuação de `testes.md` (numeração **preservada**, §22.4+): padrão de script de teste, seeds/personas, integração com CI, pentest (§22.7–22.8), o Q.A. plan-first (disciplina na skill **schematize-qa**) e o Makefile padrão (§23).
...
O fluxo de Q.A. plan-first (planejar tudo → gerar o MD de passo a passo → **aprovar antes de rodar** → executar faseado ou de uma vez com watchdog → coletar `summary.json` → travar nos gates) vive agora na skill dedicada **schematize-qa** (`/qa-plan` → `/qa-run`).

No recorte Node/TS, `/node-qa` é o wrapper fino que aplica essa disciplina com o ferramental da plataforma — `vitest`/`npm test`/`node:test`, cobertura (`c8`/`vitest --coverage`), os scripts de teste desta skill e o pentest da schematize-pentest. Nada de Q.A. roda sem plano aprovado registrado no `<projeto>_archive/qa/`.
